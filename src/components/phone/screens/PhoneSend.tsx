...
export function PhoneSend() {
  return (
    <div className="flex h-full flex-col bg-page">
      ...
      <div className="flex-1 space-y-2.5 overflow-hidden px-4 pt-0.5">
        {/* recipient cashtag */}
        ...
        <div className="flex items-center gap-2.5 rounded-[13px] border-2 border-brand bg-white p-2">
          <InitialAvatar initials="AO" size={32} />
          <div className="min-w-0 flex-1">
            <p className="flex items-center gap-1 text-[10px] font-bold text-ink">
              Ada Obi <VerifiedBadge />
            </p>
            <p className="text-[8.5px] font-semibold text-brand">@ada</p>
          </div>
          <span className="flex size-[18px] items-center justify-center rounded-full bg-brand">
            <Check size={10} className="text-white" strokeWidth={3} />
          </span>
        </div>

        {/* amount */}
        <div className="rounded-[15px] border border-line bg-white p-3 text-center">
          <p className="text-[8.5px] font-semibold text-ink-soft">Enter Amount to Send</p>
          <p className="pt-0.5 text-[21px] font-bold tracking-tight text-ink">₦10,000.00</p>
          <div className="mt-2 grid grid-cols-4 gap-1.5">
            {AMOUNTS.map((amount) => (
              <span
                key={amount}
                className={`rounded-full py-1 text-[7.5px] font-bold ${
                  amount === '₦10,000'
        ...
        {/* note */}
        <div className="rounded-[12px] border border-line bg-white px-3 py-2 text-[9px] font-medium text-ink-soft">
          Add a note <span className="text-ink-soft/50">· “Lunch money 🍜”</span>
        </div>

        {/* CTA */}
        <button className="flex h-10 w-full items-center justify-center gap-1.5 rounded-[14px] bg-brand text-[11px] font-bold text-white shadow-[0_10px_22px_-8px_rgba(46,69,244,0.65)]">
          <Send size={12} /> Send ₦10,000
        </button>
        <p className="flex items-center justify-center gap-1 text-center text-[7px] font-medium text-ink-soft">
          <ShieldCheck size={9} className="text-good" />
          Directly debited and credited to recipient Cashtag instantly.
        </p>
      </div>

      <PhoneTabBar active="send" />
    </div>
  );
}
